...
> - Трекинг заказа от службы доставки (получение новых данных о статусе);
...
* `10%` — конверсия в корзину из карточки товара — отношение количества добавлений товара в корзину к количеству уникальных посетителей, которые просмотрели карточку товара. Эта величина колеблится в диапазоне от 5% до 15%, используем среднее — 10%.
* 10% —  конверсии от покупки к оставленному отзыву.
...
1813 / 234 = 8 раз в среднем пользователь добавляет товар в корзину в течение дня
 ...
 Показатель отказа (Bounce Rate) 7% следовательно конверсия из авторизации в поиск 93%, т.е 7% от DAU при заходе на главную страницу покидают сайт, ничего не сделав, а 93% хотя бы пользуются поиском или каталогом.

Воронка конверсий eCommerce [[16](#sources)]
![alt text](images/image-9w.png)

---
#### Операционные показатели [[21](#sources)]
| Год  | Млн. п/г   | YoY  | MAU | Млн. т/г  | Частота заказов в год |
| ---- | -----      | ---- | --- | -----     | ---  |
| 2023 | 46.1       | +31% | 128 | 966       | 21   |
| 2022 | 35,2       | +37% | 98  | 465.4     | 13.2 |
| `2021` | 25,6       | +86% | 70  | 223.3     | 8.7  |
---
...
*MAU за 2023 = 128 млн. 70/128=55% Следовательно RPS 11k RPS 0.55 = 6k RPS на момент 2021года 50 х 0.55= 27.5 пик*
...
| ---- | ---- | ---- |
| Просмотр каталога и поиск | $`\frac{6kRPS\cdot 60 \cdot 60 \cdot 24}{93\% \cdot DAU} `$ | 37 |
| Заказ товара | $`\frac{611 780}{93\% \cdot  DAU}`$ | 0,04 |
| Добавление в корзину | $`\frac{0.04}{30\%}`$ | 0.13 |
| Просмотр отдельного товара | $`\frac{0.13}{10\%}`$| 1,3 |
| Отзывы и рейтинг | | | 
