...
`x9,15` — множитель от средней нагрузки в 2021г к пиковой в 2023г: $5 \cdot 1.83 = 9.15$ 
...
| ---------------------- | --------------- | ----------------------------------- | ------ |
| **PROFILE**            | 323             | $`30\cdot 10^6`$ $`(MAU\cdot 2)`$   | 9 Гб   |
| **PRODUCT**            | 1264            | $`250\cdot 10^6`$                   | 300 Гб |
| **COMMENT**            | 308; 52         | $`128.5\cdot 10^6; 53.5\cdot 10^6`$ | 40 Гб  |
| **ORDER_INFO**         | 84              | $`356.8\cdot10^6`$                  | 28 Гб  |
| **STATUS**             | 12              | $`10`$                              | -      |
| **ORDER_ITEM**         | 56              | $`600\cdot10^6`$                    | 32 Гб  |
| **ADDRESS**            | 81              | $`45\cdot 10^6`$  $`(MAU\cdot3) `$  | 4 Гб   |
| **CATEGORY**           | 24              | $`10^4`$                            | -      |
| **CART**               | 33              | $`15\cdot 10^6`$                    | 1 Гб   |
| **SHOPPING_CART_ITEM** | 52              | $`15\cdot 10^7 `$                   | 8 Гб   |
...
###  Table $`\longleftrightarrow`$ Database
...
* [neo4j-go-driver](https://neo4j.com/docs/go-manual/current/) — драйвер для графовой БД Neo4j.
* [go-ceph](https://pkg.go.dev/github.com/ceph/go-ceph) —   драйвер для сетевого файлового хранилища **CEPH**.
* [pgx](https://pkg.go.dev/github.com/jackc/pgx) — драйвер для основной БД PostgreSQL.
* [go-elasticsearch](https://www.elastic.co/guide/en/elasticsearch/client/go-api/current/getting-started-go.html) — драйвер для БД поиска Elasticsearch.
...
**CEPH** — программно определяемая система хранения данных, объединяет несколько дисков обычной сетью. Для хранения фотографий будем использовать CEPH, суммарный объём `150 ТБ`, с возможностью масштабироваться до `300 ТБ`.

---

### Шардинг — PRODUCT Table 
...
Shard Key — `product_id`. Берём остаток от деления по модулю на 1024 от md5(`product_id`) и в зависимости от интервала куда попадает резуьтат 0..127, 128..255, 256..383 и т.д. выбираем физисеский адрес шарда. Следовательно решардинг делаем в рамках одного кластера и для этого делим предыдущие интервалы попалам, получаем: 0..63, 64..127, 128..191 и т.д.
...
Паттерн работы с данными. Пишем в мастер.
...
Решаемые проблемы: если много удаляем/апдейтим записи в базе, то vacuum может работать довольно долго.
...
В итоге, для таблицы `PRODUCT` чтобы держать 65K RPS можно сделать 64 действующих шарда и 16 шардов в запас для решардинга. Один шард будет иметь: 1 синхронную реплику и 2 асинхронных реплики, 100 партиций: product_0, product_1, product_2...
...
Используем Батчинг + сжатие. Этот паттерн решает проблему потери событий. 
...
**OLAP** — В Clickhouse пишутся асинхронно данные для аналитики и обучения ML.
ML вектора для рекомендаций будут храниться в LanceDB.
...
![https://s.icepanel.io/4CQ6UE87YvPjL5/WNMj](images/search.png)
...
![https://s.icepanel.io/4CQ6UE87YvPjL5/WNMj](images/shema.png)
...
43. https://habr.com/ru/companies/ozontech/articles/667600/
44. https://habr.com/ru/companies/uzum/articles/753094/
45. https://habr.com/ru/articles/545634/
46. https://habr.com/ru/companies/sportmaster_lab/articles/756004/